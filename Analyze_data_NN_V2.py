...
from Coverage_check import Coverage_analysis

def NN_analyzer(variables, pod5_dr, bam_fh, read_id, sig_map_refiner, model, reference, labels_mod):
    ...
    reference_track_mod = np.zeros([len(reference), labels_mod]) # matrix to track the modification     
    Track_coverage = np.zeros([len(reference)])
    ...
    for name_id in read_id[start_reads: end_reads]:
        ...
        try:
            ...
            end_of_sequence = start_of_mapping.ref_reg.len
        ...
        if Error_read == False:
            ...
            try:
                ...
                for kk in range(N_segments): #
                    ...
                    full_position = np.sum(Input_2[kk], axis = 1)
                ...
                full_position = np.sum(Input_2[N_segments], axis = 1)
                full_position = np.where(full_position> 0.5)[0]
                ...
                if (len(Final_seq_binary) - len(seq_resquigle)) != 0:
                    ...
                    Track_coverage[ int(position_adjusting): int(position_adjusting) +  int(end_of_sequence)] += 1
    ...
    # //////// caculate the weight for each base calculating the distance from avarage covarage ////////
    # the median seems to work bad in few cases. need to check where is the error.

    N_tot_analyzed = np.abs(end_reads - start_reads - N_miss)
    Final_results = Coverage_analysis(N_tot_analyzed, reference_track_mod, Track_coverage, threshold = 0.2)

    print("analysis finished")
    print("Total data to analyize:", np.abs(end_reads - start_reads))
    print("data analyized:", N_tot_analyzed)
